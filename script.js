...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const getGuidedHTML = () => {
        ...
        return `<div class="timeline">${timelineHTML}<div class="timeline-line"><div id="timeline-progress"></div></div></div><div id="guided-form-container"></div><div id="results-container" class="hidden mt-12"></div>`;
    };
    ...
    const renderRepaymentChart = () => { if (state.chart) { state.chart.destroy(); } const ctx = document.getElementById('repaymentChart')?.getContext('2d'); if (!ctx || !state.calculation.selectedOffer) return; const { loanAmount, loanTerm } = state.formData; const { rate } = state.calculation.selectedOffer; const schedule = Array.from({ length: loanTerm }, (_, i) => calculateAmortization(loanAmount, rate, loanTerm, i + 1)); state.chart = new Chart(ctx, { type: 'bar', data: { labels: schedule.map(item => `Rok ${item.year}`), datasets: [{ label: 'Úroky', data: schedule.map(item => item.interest), backgroundColor: '#fca5a5' }, { label: 'Jistina', data: schedule.map(item => item.principal), backgroundColor: '#60a5fa' }] }, options: { responsive: true, maintainAspectRatio: false, scales: { x: { stacked: true }, y: { stacked: true, ticks: { callback: value => formatNumber(value) } } } } }); };
    ...
    const handleInput = (e) => { const { id, value, type } = e.target; const baseId = id.replace('-input', ''); if (state.formData.hasOwnProperty(baseId)) { const parsedValue = (type === 'range' || id.endsWith('-input')) ? parseNumber(value) : value; state.formData[baseId] = parsedValue; if (type === 'range') { const input = document.getElementById(`${baseId}-input`); if(input) input.value = formatNumber(parsedValue, false); } else if (id.endsWith('-input')) { const slider = document.getElementById(baseId); if(slider) slider.value = parsedValue; } debouncedCalculation(); } };
    const handleClick = (e) => {
    ...
    const handleChatMessageSend = async (message) => {
        ...
        try {
            ...
            if (data.tool === 'calculateMortgage') { Object.assign(state.formData, data.params); switchMode('express'); setTimeout(calculateRates, 200); }
