...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const CONFIG = {
        ...
        DEBOUNCE_DELAY: 500,
        AI_SUGGESTIONS: {
            initial: ["Jaké jsou aktuální úrokové sazby?", "Co je to LTV?", "Co když jsem OSVČ?"],
            contextual: {
    ...
    const getGuidedHTML = () => `
        <div id="guided-form" class="space-y-8">
            <div>
    ...
    const handleChatMessageSend = async (message) => {
        ...
        try {
            ...
            if (data.tool === 'goToCalculator') { switchMode('express'); if (data.params) { Object.assign(state.formData, data.params); DOMElements.contentContainer.innerHTML = getExpressHTML(); debouncedCalculation(); } }
