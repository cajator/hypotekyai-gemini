...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const createSlider = (id, label, value, min, max, step) => {
        ...
        return `<div class="slider-group"><div class="flex justify-between items-center mb-1"><label for="${id}" class="form-label mb-0">${label}</label><div class="flex items-center"><input type="text" id="${id}-input" value="${formatNumber(value, false)}" class="slider-value-input"><span class="font-semibold">${suffix}</span></div></div><div class="slider-container"><input type="range" id="${id}" name="${id}" min="${min}" max="${max}" value="${value}" step="${step}" class="slider-input"></div></div>`;
    };

    const getCalculatorLayout = (formHTML) => `<div class="bg-white p-6 md:p-12 rounded-2xl shadow-xl border">${formHTML}</div>`;
    const getAiLayout = (chatHTML) => `<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start"><div class="lg:col-span-2 bg-white rounded-2xl">${chatHTML}</div><div id="sidebar-container" class="lg:sticky top-28 space-y-6">${getSidebarHTML()}</div></div>`;
    const getSidebarHTML = (calc = null) => { const hasCalc = calc && calc.offers && calc.offers.length > 0; const payment = hasCalc ? formatNumber(calc.selectedOffer.monthlyPayment) : '-'; const rate = hasCalc ? `${calc.selectedOffer.rate.toFixed(2)} %` : '-'; const ltv = hasCalc ? `${Math.round((state.formData.loanAmount / state.formData.propertyValue) * 100)} %` : '-'; const allTips = (calc?.smartTip ? [calc.smartTip] : []).concat(calc?.tips || []); const tipsHTML = allTips.length > 0 ? allTips.map(tip => `<div class="mt-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-3 rounded-r-lg text-sm"><p class="font-bold">${tip.title}</p><p>${tip.message}</p></div>`).join('') : '<p class="text-sm text-gray-500 mt-4">Zadejte do chatu údaje pro modelování a zobrazení chytrých tipů.</p>'; return `<div class="bg-blue-50 p-6 rounded-2xl border border-blue-200"><h3 class="text-xl font-bold mb-4">Souhrn hypotéky</h3><div class="space-y-3"><div class="flex justify-between text-lg"><span class="text-gray-600">Měsíční splátka</span><strong id="sidebar-payment">${payment}</strong></div><div class="flex justify-between"><span class="text-gray-600">Úroková sazba</span><strong id="sidebar-rate">${rate}</strong></div><div class="flex justify-between"><span class="text-gray-600">LTV</span><strong id="sidebar-ltv">${ltv}</strong></div></div></div><div class="bg-gray-50 p-6 rounded-2xl border"><h4 class="font-bold mb-2">Graf splácení</h4><div id="sidebar-chart-container" class="relative h-48"><canvas id="sidebarChart"></canvas></div></div><div id="sidebar-tips">${tipsHTML}</div>`; };
    const getExpressHTML = () => getCalculatorLayout(`<div id="express-form" class="space-y-6">${createSlider('propertyValue','Hodnota nemovitosti',state.formData.propertyValue,500000,30000000,100000)}${createSlider('loanAmount','Chci si půjčit',state.formData.loanAmount,200000,20000000,100000)}${createSlider('income','Měsíční čistý příjem',state.formData.income,15000,300000,1000)}${createSlider('liabilities','Měsíční splátky jiných úvěrů',state.formData.liabilities,0,100000,500)}<div class="flex justify-center pt-4"><button class="nav-btn text-lg w-full md:w-auto" data-action="calculate"><span class="mr-2">Spočítat nabídky</span><div class="loading-spinner-white hidden"></div></button></div></div><div id="results-container" class="hidden mt-12"></div>`);
    const getGuidedHTML = () => getCalculatorLayout(`<div id="guided-form" class="space-y-8"><div><h3 class="form-section-heading">Parametry úvěru a nemovitosti</h3><div class="grid grid-cols-1 md:grid-cols-2 gap-6">${createSlider('propertyValue','Hodnota nemovitosti',state.formData.propertyValue,500000,30000000,100000)}${createSlider('loanAmount','Požadovaná výše úvěru',state.formData.loanAmount,200000,20000000,100000)}${createSlider('loanTerm','Délka splatnosti',state.formData.loanTerm,5,30,1)}${createSlider('fixation','Délka fixace',state.formData.fixation,3,10,1)}</div></div><div><h3 class="form-section-heading">Vaše bonita a osobní údaje</h3><div class="grid grid-cols-1 md:grid-cols-2 gap-6">${createSlider('income','Čistý měsíční příjem',state.formData.income,15000,300000,1000)}${createSlider('liabilities','Měsíční splátky jiných úvěrů',state.formData.liabilities,0,100000,500)}${createSlider('age','Věk nejstaršího žadatele',state.formData.age,18,70,1)}${createSlider('children','Počet dětí',state.formData.children,0,10,1)}</div></div><div class="flex justify-center pt-4"><button class="nav-btn text-lg w-full md:w-auto" data-action="calculate"><span class="mr-2">Spočítat a najít nabídky</span><div class="loading-spinner-white hidden ml-2"></div></button></div></div><div id="results-container" class="hidden mt-12"></div>`);
    const getAiModeHTML = () => getAiLayout(`<div class="flex flex-col h-[70vh]"><div id="chat-messages" class="flex-1 overflow-y-auto p-4 space-y-4"></div><div id="ai-suggestions" class="p-4 border-t"></div><div class="p-4 border-t flex items-center space-x-2"><input type="text" id="chat-input" class="modern-input" placeholder="Zadejte svůj dotaz..."><button id="chat-send" class="nav-btn" data-action="send-chat">Odeslat</button></div></div>`);
    ...
    const renderResults = () => {
        ...
        container.innerHTML = `<div><h3 class="text-3xl font-bold mb-6">Našli jsme pro vás tyto nabídky:</h3><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">${offersHTML}</div></div><div class="text-center mt-12 bg-white p-8 rounded-2xl shadow-lg border"><h3 class="text-2xl font-bold mb-2">Co dál?</h3><p class="text-gray-600 mb-6 max-w-2xl mx-auto">Zobrazené nabídky jsou přesnou indikací reálných sazeb, které naši specialisté v posledních dnech pro klienty s podobným profilem vyjednali. Finální sazba závisí na detailním posouzení bankou. Náš specialista s vámi probere všechny detaily a zajistí ty nejlepší možné podmínky.</p><div class="flex justify-center items-center gap-4"><button class="nav-btn bg-green-600 hover:bg-green-700 text-lg" data-action="show-lead-form">Chci nejlepší nabídku</button><button class="nav-btn bg-gray-600 hover:bg-gray-700" data-action="discuss-with-ai">Probrat s AI stratégem</button></div></div>`;
    ...
    const renderSidebarChart = () => { if (state.chart) { state.chart.destroy(); } const ctx = document.getElementById('sidebarChart')?.getContext('2d'); if (!ctx || !state.calculation.selectedOffer) return; const { loanAmount, loanTerm } = state.formData; const { rate } = state.calculation.selectedOffer; if (loanTerm <= 0) return; const schedule = Array.from({ length: loanTerm }, (_, i) => calculateAmortization(loanAmount, rate, loanTerm, i + 1)); state.chart = new Chart(ctx, { type: 'bar', data: { labels: schedule.map(item => item.year), datasets: [{ label: 'Úroky', data: schedule.map(item => item.interest), backgroundColor: '#fca5a5' }, { label: 'Jistina', data: schedule.map(item => item.principal), backgroundColor: '#60a5fa' }] }, options: { responsive: true, maintainAspectRatio: false, scales: { x: { stacked: true }, y: { stacked: true, ticks: { display: false } } }, plugins: { legend: { display: false } } } }); };
    ...
    const handleClick = (e) => {
        ...
        else if (target.matches('.offer-card')) { document.querySelectorAll('.offer-card').forEach(c => c.classList.remove('selected')); target.classList.add('selected'); state.calculation.selectedOffer = state.calculation.offers.find(o => o.id === target.dataset.offerId); renderSidebar(); }
    };
    ...
    const handleChatMessageSend = async (message) => {
        ...
        try {
            ...
            if (data.tool === 'modelScenario') { state.formData = {...state.formData, ...data.params}; addChatMessage('Rozumím, moment. Počítám nový scénář a aktualizuji data vpravo.', 'ai'); const silentButton = document.createElement('button'); calculateRates(silentButton).then(() => { renderSidebar(state.calculation); }); }
    ...
    const switchMode = (mode) => {
        ...
        else if (mode === 'ai') { container.innerHTML = getAiModeHTML(); addChatMessage('Dobrý den! Jsem Hypoteční stratég. Zeptejte se na cokoliv, nebo mi napište parametry a já Vám je namodeluji v panelu vpravo.', 'ai'); generateAISuggestions(); document.getElementById('chat-input')?.addEventListener('keydown', handleChatEnter); }
