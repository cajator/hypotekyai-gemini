...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const getCalculatorLayout = (formHTML) => {
        return `<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start"><div class="lg:col-span-2 bg-white rounded-2xl shadow-xl border">${formHTML}</div><div id="sidebar-container" class="lg:sticky top-28 space-y-6">${getSidebarHTML()}</div></div><div id="results-container" class="hidden mt-12"></div>`;
    };
    ...
    const renderResults = () => {
        ...
        container.innerHTML = `<div class="lg:col-span-3"><h3 class="text-3xl font-bold mb-6">Našli jsme pro vás tyto nabídky:</h3><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">${offersHTML}</div></div>`;
        
        document.getElementById('sidebar-container').innerHTML = `<div class="bg-blue-50 p-6 rounded-2xl border border-blue-200"><h4 class="text-xl font-bold mb-4">Přehled a skóre vaší žádosti</h4><div class="space-y-3">${scoreHTML('LTV', approvability.ltv, 'bg-green-500')}${scoreHTML('DSTI', approvability.dsti, 'bg-yellow-500')}${scoreHTML('Bonita', approvability.bonita, 'bg-green-500')}</div><h4 class="text-lg font-bold mt-6 mb-2">Celková šance: <span class="text-2xl font-bold text-green-600">${approvability.total}%</span></h4><div class="approvability-bar-bg"><div class="approvability-bar bg-green-500" style="width: ${approvability.total}%"></div></div>${allTipsHTML}</div><div class="text-center mt-6"><button class="nav-btn bg-green-600 hover:bg-green-700 text-lg" data-action="show-lead-form">Chci nejlepší nabídku</button></div>`;
    ...
    const handleClick = (e) => {
        ...
        else if (target.matches('.offer-card')) { document.querySelectorAll('.offer-card').forEach(c => c.classList.remove('selected')); target.classList.add('selected'); state.calculation.selectedOffer = state.calculation.offers.find(o => o.id === target.dataset.offerId); document.getElementById('sidebar-container').innerHTML = getSidebarHTML(state.calculation); setTimeout(renderSidebarChart, 50); }
    };
    ...
    const handleChatMessageSend = async (message) => {
        ...
        try {
            ...
            if (data.tool === 'startContactForm') { addChatMessage(data.response, 'ai'); state.chatFormState = 'awaiting_name'; }
