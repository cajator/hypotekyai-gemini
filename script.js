...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderResults = () => {
        ...
        container.innerHTML = `
            <div><h3 class="text-3xl font-bold mb-6">Našli jsme pro vás tyto nabídky:</h3><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">${offersHTML}</div></div>
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-12">
                <div class="lg:col-span-2 bg-gray-50 p-6 rounded-xl"><h4 class="text-xl font-bold mb-4">Vývoj splácení v čase</h4><canvas id="repaymentChart"></canvas></div>
        ...
        if (firstCard) {
            ...
            state.calculation.selectedOffer = state.calculation.offers.find(o => o.id === firstCard.dataset.offerId);
        }
    ...
    const generateAISuggestions = () => {
        ...
        let suggestions = [...CONFIG.AI_SUGGESTIONS.initial];
        if (state.calculation.offers && state.calculation.offers.length > 0) {
            suggestions = [...CONFIG.AI_SUGGESTIONS.contextual.base];
