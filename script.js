...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- STATE MANAGEMENT ---
    const state = {
        mode: 'express', // Default mode is Express
        formData: {
    ...
    const getExpressHTML = () => {
        const formHTML = `<div id="express-form" class="space-y-6 p-6 md:p-8">${createSlider('propertyValue','Hodnota nemovitosti',state.formData.propertyValue,500000,30000000,100000)}${createSlider('loanAmount','Chci si půjčit',state.formData.loanAmount,200000,20000000,100000)}${createSlider('income','Měsíční čistý příjem',state.formData.income,15000,300000,1000)}${createSlider('liabilities','Měsíční splátky jiných úvěrů',state.formData.liabilities,0,100000,500)}</div>`;
        return getCalculatorLayout(formHTML);
    };

    const getGuidedHTML = () => {
        const formHTML = `<div id="guided-form" class="space-y-8 p-6 md:p-8"><div><h3 class="form-section-heading">Parametry úvěru</h3><div class="grid grid-cols-1 md:grid-cols-2 gap-6">${createSlider('propertyValue','Hodnota nemovitosti',state.formData.propertyValue,500000,30000000,100000)}${createSlider('loanAmount','Požadovaná výše úvěru',state.formData.loanAmount,200000,20000000,100000)}${createSlider('loanTerm','Délka splatnosti',state.formData.loanTerm,5,30,1)}${createSlider('fixation','Délka fixace',state.formData.fixation,3,10,1)}</div></div><div><h3 class="form-section-heading">Vaše bonita</h3><div class="grid grid-cols-1 md:grid-cols-2 gap-6">${createSlider('income','Čistý měsíční příjem',state.formData.income,15000,300000,1000)}${createSlider('liabilities','Měsíční splátky jiných úvěrů',state.formData.liabilities,0,100000,500)}${createSlider('age','Věk nejstaršího žadatele',state.formData.age,18,70,1)}${createSlider('children','Počet dětí',state.formData.children,0,10,1)}</div></div><div class="flex justify-center pt-4"><button class="nav-btn text-lg w-full md:w-auto" data-action="calculate"><span class="mr-2">Spočítat a najít nabídky</span><div class="loading-spinner-white hidden ml-2"></div></button></div></div>`;
        return getCalculatorLayout(formHTML);
    };

    const getAiModeHTML = () => `<div class="bg-white rounded-2xl"><div class="flex flex-col h-[70vh]"><div id="chat-messages" class="flex-1 overflow-y-auto p-4 space-y-4"></div><div id="ai-suggestions" class="p-4 border-t"></div><div class="p-4 border-t flex items-center space-x-2"><input type="text" id="chat-input" class="modern-input" placeholder="Zadejte svůj dotaz..."><button id="chat-send" class="nav-btn">Odeslat</button></div></div></div>`;
    
    // --- UI RENDER FUNCTIONS ---
    const renderSidebar = () => { document.getElementById('sidebar-container').innerHTML = getSidebarHTML(state.calculation); setTimeout(renderSidebarChart, 0); };
    const renderResults = () => {
    ...
    // --- EVENT HANDLERS ---
    const handleInput = (e) => { const { id, value, type, name } = e.target; const baseId = id.replace('-input', ''); if (state.formData.hasOwnProperty(baseId)) { const parsedValue = (type === 'range' || id.endsWith('-input')) ? parseNumber(value) : value; state.formData[baseId] = parsedValue; if (type === 'range') { const input = document.getElementById(`${baseId}-input`); if(input) input.value = formatNumber(parsedValue, false); } else if (id.endsWith('-input')) { const slider = document.getElementById(baseId); if(slider) slider.value = parsedValue; } } };
    const handleClick = (e) => {
    ...
    const handleChatMessageSend = async (message) => {
        ...
        try {
            ...
            if (data.tool === 'goToCalculator') { switchMode('express'); if (data.params) { Object.assign(state.formData, data.params); DOMElements.contentContainer.innerHTML = getExpressHTML(); } }
            else if (data.tool === 'goToContact') { addChatMessage(data.response, 'ai'); scrollToAndShow('#kontakt'); }
    ...
    const init = () => {
        ...
        let count = 147; setInterval(() => { 
            const hour = new Date().getHours();
            const baseCount = (hour >= 9 && hour <= 17) ? 150 : 80;
            count = Math.max(baseCount - 20, Math.min(baseCount + 20, count + (Math.floor(Math.random() * 5) - 2))); 
